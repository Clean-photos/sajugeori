...
띠 관계: ${PET_BRANCH_HINT[facts.relation.branch]}
오행 흐름(아이의 속마음): ${PET_FLOW_HINT[facts.relation.flow]}
...
${facts.lifestyle.shared
  ...
  const prompt = `당신은 따뜻한 명리학 상담가입니다. 아래는 사주 엔진이 계산한 '반려동물과 주인의 궁합' 데이터입니다.
이 데이터에 근거해, 반려동물을 사랑하는 주인이 읽고 흐뭇하고 감동할 궁합 리포트를 작성하세요.

${engineSummary}

이것은 유료 프리미엄 리포트입니다. 각 섹션을 충분한 분량으로 깊이 있게 쓰세요.
...
반드시 ${sp.label}다운 장면으로 묘사하세요. 위 '${sp.label}의 행동 심리'에 나온 습성과 애정 표현 방식을 사주의 기운과 엮어 서술하고, 다른 종의 행동(${facts.species === "cat" ? "산책 줄을 매고 함께 걷기, 현관에서 꼬리 흔들며 반기기 등" : "캣타워에 오르기, 골골 소리 내기 등"})은 쓰지 마세요.

【 집사님의 사주 】
주인의 일간·오행·강약을 근거로 8~10문장의 충실한 분석을 쓰세요. 타고난 기질과 성향, 사람을 대하는 방식, 어떤 상황에서 힘을 얻고 어떤 상황에서 지치는지, 신강·신약이 일상에서 어떻게 드러나는지를 차근차근 풀어 주세요. 그리고 그런 집사님이 왜 반려동물과 함께 사는 삶에 잘 어울리는지, 아이를 어떤 방식으로 사랑하는 사람인지까지 이어서 서술해 주세요. 이 섹션은 집사님 본인에 대한 깊은 이해를 드리는 부분이니 충분히 정성껏 쓰세요.

【 둘의 케미 】
위 '띠 관계'와 '기운 보완' 힌트에 근거해 8~10문장으로 깊이 있게 쓰세요. 두 사람의 기운이 어떻게 맞물리는지 원리를 먼저 설명하고, 그것이 일상에서 어떤 장면으로 나타나는지(함께 있을 때의 공기, 집사님이 힘들 때 아이가 보이는 반응, 아이가 신났을 때 집사님이 느끼는 것 등) 구체적으로 그려 주세요. 이 만남이 왜 서로에게 좋은 인연인지, 시간이 흐를수록 어떤 관계로 깊어질지도 함께 짚어 주세요. 이 섹션은 리포트의 중심이니 가장 공들여 쓰세요.
...
또한 위 '${sp.label}의 행동 심리'를 근거로, ${petName}가 그 마음을 실제로 어떤 행동으로 드러내는지 한두 장면을 구체적으로 넣어 주세요. ${facts.species === "cat"
      ? "예를 들어 슬로우 블링크, 머리를 비비는 것, 곁에서 골골거리는 것처럼 고양이 특유의 신호로 표현하세요."
      : "예를 들어 낯선 것을 만났을 때 집사님을 돌아보는 것, 발치에 붙어 앉는 것처럼 강아지 특유의 신호로 표현하세요."} 연구로 확인된 사실이라는 점을 자연스럽게 곁들이면 더 좋습니다.
...
- 따뜻하고 다정한 존댓말. 아이를 '${petName}'로 부를 것. 이모지 사용 금지.
- ${petName}는 ${sp.label}입니다. 전 섹션에 걸쳐 ${sp.label}의 실제 습성에 맞는 장면만 쓰고, 다른 종의 행동을 섞지 마세요.
