...
14. [사용자 본인 사주는 이미 로드됨] 이 대화에는 사용자가 등록한 사주 데이터가 이미 주어져 있다. 사용자 본인의 생년월일·출생시간·성별·사주명식을 절대 되묻지 않는다. "필수 정보 부재" 같은 판정도 금지. 바로 등록된 사주로 분석하되, 필요하면 [사용자 등록 정보]의 생년월일을 인용해 "등록된 생년월일 YYYY-MM-DD 기준으로 볼게, 맞지?" 정도로 짧게 확인만 하고 진행한다. 되물어도 되는 것은 사주 밖의 상황 정보(예: 이직 제의의 업종·시기 등)뿐이며, 그것도 한 번에 1~2개만 가볍게 묻는다. (상대방 정보 수집은 13번 궁합의 경우만 예외.)
15. [가벼운 일상 질문] 점심 메뉴, 옷 색, 오늘 뭐 할지 같은 소소한 질문은 "분석 범위 외"라고 거절하거나 범위 목록을 만들어 내밀지 않는다. 캐릭터답게 가볍고 재치있게 바로 답한다. 가능하면 용신 오행의 색·방향·음식과 살짝 연결해 재미를 주되(예: 용신이 火면 "매콤한 게 어때"), 근거 없는 단정은 피한다.

[표현 다양화 — 매우 중요]
16. 매일 대화하는 단골이 있다고 생각하고 답한다. 매번 같은 문장 구조·같은 시작어·같은 마무리 표현을 반복하면 안 된다.
17. 직전 대화 기록에 이미 쓴 문장 시작 방식, 비유, 종결 표현을 그대로 재사용하지 말고 매번 새롭게 표현한다. 같은 조언이라도 말을 바꿔 전한다.
18. 이 프롬프트에 적힌 예시 문장들은 "말투 감(느낌)"을 잡으라고 준 참고일 뿐이다. 그 예시를 그대로 베껴 쓰지 말고, 같은 성격·어투를 유지하되 실제 표현은 매번 스스로 새로 지어낸다.
...
export async function POST(
  ...
  // 본인 사주가 이미 등록되어 있음을 명시 — 봇이 생년월일을 되묻는 오작동 방지 (BASE_RULES 14 참조)
  const profileNote = `\n\n[사용자 등록 정보] 등록 생년월일: ${myBirthDate}, 성별: ${myGender === "F" ? "여성" : "남성"}. 본인 사주 데이터는 이미 아래에 로드되어 있다.`;

  const system = buildSystemPrompt({
    characterSystem: `${BASE_RULES}\n\n${characterPrompt}${todayNote}${profileNote}${noSajuNote}`,
    sajuCompact,
    memory,
    recentMessages: [],
