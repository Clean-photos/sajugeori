...
다음 JSON으로만 응답하세요:
{
  ...
  "body": "정확히 4개 문단으로 나눠 각 문단 사이를 빈 줄(줄바꿈 두 번, \\n\\n)로 구분할 것. 각 문단은 '1. ' '2. ' '3. ' '4. ' 번호로 시작하고, 번호 바로 뒤에는 그 문단 내용을 압축한 한 줄 요약 문장(소제목이 아니라 완성된 서술문)을 붙인 뒤, 이어서 본문 문장을 쓸 것. 1문단: 전체 설계 요약(첫 문장은 '이 운명은'으로 시작). 2문단: 명식 근거(오행·십성을 실명으로 인용 — 숫자만 나열하지 말고 그 구조가 뜻하는 바를 설명). 3문단: 실생활에서의 발현. 4문단: 이 설계도의 핵심 과제와 네 개의 축(생계·관계·신체·공간과 시간)으로의 연결. 각 문단은 2~3문장(요약 줄 제외)."
}
${COMMON_RULES}`;
}
