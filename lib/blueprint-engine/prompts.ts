...
- 신살의 한자는 사실 시트에 이미 괄호로 붙어 있다 — 그 한자를 그대로 쓰고 다른 한자를 새로 짓지 말 것(예: 고신살은 시트가 준 한자만 쓸 것).
...
다음 JSON으로만 응답하세요:
{
  ...
  "body": "정확히 4개 문단으로 나눠 각 문단 사이를 빈 줄(줄바꿈 두 번, \\n\\n)로 구분할 것. 각 문단은 '숫자. 요약문장\\n본문문장들' 형식이어야 한다 — 번호 뒤에 그 문단 전체를 압축한 완결된 한 문장(요약문장)을 쓰고, 그 요약문장이 끝나면 줄바꿈(\\n) 하나를 넣은 뒤에야 본문 문장들을 이어 쓸 것. §3-1(CoS 실물 재검증, 2026-09-13): 예전엔 번호 뒤에 바로 본문이 시작돼(예: '1. 이 운명은 힘의 분산 없이...') 요약 자리가 비어 있었다 — 요약문장은 본문 첫 문장을 그대로 가져오거나 그 시작 부분만 잘라 쓰는 것이 아니라, 그 문단의 결론만 담은 독립된 짧은 문장이어야 한다. 예시(1문단): '1. 넓게 벌이기보다 한 방향으로 깊게 파는 구조입니다.\\n이 운명은 힘의 분산 없이 특정 방향으로 자원을 집중시켜야만 설계가 작동합니다...'처럼 요약과 본문이 뚜렷이 다른 문장이어야 한다(요약: 결론 한 줄 / 본문: 근거·설명). 1문단: 전체 설계 요약(본문 첫 문장은 '이 운명은'으로 시작). 2문단: 명식 근거(오행·십성을 실명으로 인용하되, '토2.24'처럼 소수점 가중치 숫자를 그대로 나열하지 말 것 — 단위를 알 수 없는 숫자다. 대신 '많다/적다/거의 없다' 같은 상대적 서술이나 '전혀 없다'처럼 사실 시트가 이미 강조한 표현만 쓸 것). 3문단: 실생활에서의 발현. 4문단: 이 설계도의 핵심 과제와 네 개의 축(생계·관계·신체·공간과 시간)으로의 연결. 각 문단은 요약문장 1개 + 본문 2~3문장. §2-2(CoS 실물 재검증, 2026-09-13): '축적력 70'·'회복력 40'처럼 6대 지표 이름과 점수를 요약문장·본문 어디에도 직접 부르지 말 것 — 이 규칙이 원래 buildAxisGroupPrompt·buildClosingPrompt에만 있고 여기(운명총론)에는 빠져 있어, 총론 첫 문단부터 지표 수치가 그대로 새는 사고가 실측됐다. 지표가 뜻하는 행동 특징(예: '오래 버티는 힘이 강하다')과 오행·십성 실명만 근거로 쓸 것."
}
${COMMON_RULES}`;
}
...
- metrics: 이 판정과 관련된 사실 시트의 구체 수치를 인용(오행 개수, 지표 점수 등 최소 2개, 1문장) — 수치 근거는 이 필드에만 쓴다. §1(CoS 지시): 용신을 언급할 때 "용신 토·금" "기신 화·수"처럼 한 오행 세트로 단정하지 말 것 — 사실 시트의 "억부 용신 / 조후 용신 / 종합" 세 값을 그대로 쓰고, "기신"도 억부 기준임을 밝힐 것(조후상 필요한 오행이 기신에 겹치면 그 오행은 무조건 피할 것으로 쓰지 말 것).
- why: 명식의 어느 글자·구조 때문인지, 간지를 실명으로 인용해 2~3문장 이내로 설명. §3-2(CEO 지시): "연결력 62는..."처럼 6대 지표 이름과 점수를 이 필드(및 scenes·counterEvidence·actions)에서 직접 부르지 말 것 — 수치는 metrics 필드가 이미 담당한다. 여기서는 그 수치가 뜻하는 행동 특징을 문장으로 풀어 쓸 것("당신은 이런 특징이 있다"는 서술로).
