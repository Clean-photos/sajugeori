 ...
 * 다시 보내 — 앞 요청(A)이 끝나면 뒤 요청(B)이 그 결과를 이어받게 한다.
 * 새 LLM 호출을 만들어내지 않는다 — 매 재시도도 startAttempt를 다시 타므로,
 * A가 아직 진행 중이면 여전히 busy로 막히고 재시도만 반복된다.
 */
...
export async function postWithBusyRetry<T = unknown>(
  url: string,
  body: unknown,
