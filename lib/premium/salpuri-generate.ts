...
// Vercel Hobby의 60초 상한을 넘길 위험이 크다(궁합에서 72.6초 실측). 2콜→3콜로
// 쪼갰지만, "검출된 신살마다 3~4문장씩"이 신살 개수에 비례해 무한정 길어지는
// 구조라 신살이 많이 검출된 사주(실측 7개)는 4콜로 쪼개도 개별 콜 하나가
// 여전히 오래 걸려 60초를 넘겼다(2026-08-20 실측, 504). 신살 개수에 따라
// 문장 수 자체를 줄여 콜 하나의 최대 소요시간에 상한을 둔다.
 ...
 * salCount: 검출된 신살 개수. 많을수록(6개 이상) 항목당 문장 수를 줄여
 * 콜 하나의 소요 시간이 신살 개수에 비례해 무한정 늘어나지 않게 막는다.
 */
export async function generateSalpuriReport(engineSummary: string, salCount: number): Promise<string> {
  const dense = salCount >= 6;
  const listSentences = dense ? "2~3문장" : "3~4문장";
  const shortSentences = dense ? "2문장" : "3~4문장";
...
【 내 사주의 살 】
(검출된 신살을 하나씩 짚어 주되, 각 살마다 ${listSentences}: 그 살이 어느 자리(연지·월지·일지·시지)에 있는지, 그래서 어떤 영역에 작용하는지, 이 사람 사주 전체(일간·신강신약) 맥락에서 어떤 의미를 갖는지. 연지=조상·초년, 월지=부모·사회활동, 일지=배우자·본인, 시지=자식·말년. 검출된 살이 없으면 "뚜렷한 신살이 없다"는 것이 무엇을 뜻하는지 5~6문장으로 충실히 설명할 것.)
...
【 강점으로 쓰는 법 】
(검출된 살 하나하나가 지닌 긍정적 면과 그것을 살릴 수 있는 구체적 방향을 각 ${shortSentences}씩. 실제 직업·상황 예시를 곁들일 것.)
...
【 조심할 지점 】
(각 살의 그림자와 실제로 조심할 상황을 각 ${shortSentences}씩. 겁주지 말고 담담하게, 구체적인 생활 장면으로.)
