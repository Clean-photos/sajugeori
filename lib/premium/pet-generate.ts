...
- 동물행동학 근거는 위 '행동 심리' 목록에 있는 내용만 쓰세요. 목록 문장에 이미 괄호로 출처가 달려 있으면((Horn 외, PLoS ONE, 2013) 등) 저자·저널·연도를 바꾸지 말고 그대로 옮겨 쓰고, 출처가 없는 항목만 '~라고 알려져 있다' 정도로 담백하게 쓰세요. 목록에 없는 새 논문·연구·통계·수치는 절대 지어내지 마세요.
...
- 일간·오행 등 사주 용어가 나오면 처음 언급하는 자리에서 괄호 안에 쉬운 현대식 설명을 짧게 덧붙일 것. 예: 병(丙, 태양처럼 밝고 뜨거운 불의 기운), 신강(身强, 타고난 기운이 단단하고 잘 흔들리지 않는 상태). 전문 용어를 처음 접하는 독자도 이해할 수 있도록 최대한 친절하게 풀어 쓸 것. R4(CoS+CEO 실물 확인, 2026-09-08): 같은 용어가 이 응답 안에서 다시 나오면 그 뒤로는 설명을 반복하지 말고 이름만 쓸 것.
...
[${sp.label}의 행동 심리 — 동물행동학에서 알려진 경향. 사주 해석에 이 결을 반드시 얹을 것]
...
export async function generatePetReport(
  ...
  // 2콜(각 4200토큰)로는 LLM 생성만 37초가 걸려(2026-08-25 실측) DB 왕복·콜드스타트를
  // 더하면 Vercel 60초 상한에 위험할 만큼 가깝다. 프리미엄 사주가 동일한 2콜×4200
  // 구성에서 실제로 타임아웃을 냈으므로, 같은 방식으로 4콜로 쪼갰었다.
  // §C 🔴2(CoS 실물 확인, 2026-09-23): 그래도 55.9~56.6초로 상한에 바짝 붙었다.
  // 4콜 중 '아이의 사주'+'집사님의 사주' 두 섹션을 한 콜(3400토큰)에 몰아둔 것이
  // 가장 무거워, 이 콜을 둘로 더 쪼개 총 5콜 병렬로 늘린다.
  const head = `당신은 따뜻한 명리학 상담가입니다. 아래는 실제로 계산한 '반려동물과 주인의 궁합' 데이터입니다.
이 데이터에 근거해, 반려동물을 사랑하는 주인이 읽고 흐뭇하고 감동할 유료 프리미엄 궁합 리포트의
일부를 작성하세요. 990원짜리 무료 버전과는 분량·깊이가 확연히 달라야 합니다.

${engineSummary}

다음 형식으로 정확히 작성하세요:
`;
  const tail = `${COMMON_RULES}
- 아이를 '${petName}'로 부를 것.
- ${petName}는 ${sp.label}입니다. ${sp.label}의 실제 습성에 맞는 장면만 쓰고, 다른 종의 행동을 섞지 마세요.`;
...
또한 위 '${sp.label}의 행동 심리'를 근거로, ${petName}가 그 마음을 실제로 어떤 행동으로 드러내는지 한두 장면을 구체적으로 넣어 주세요. ${loveSignNote}
${tail}`, 2600),
