...
- R3(CoS+CEO 실물 확인, 2026-09-08): 날짜 점수는 만점이 정해진 척도가 아니라 날짜끼리 상대 비교용 숫자다 — "7.5점" 처럼 점수를 그대로 인용하지 말 것. 대신 "이 날이 왜 좋은지/피해야 하는지"를 합충·용신 근거로 풀어 쓸 것.
...
- 일간·용신·합충 등 사주 용어가 나오면 처음 언급하는 자리에서 괄호 안에 쉬운 현대식 설명을 짧게 덧붙일 것. 예: 계수(癸水, 유연하고 합리적인 물의 기운), 용신(用神, 이 사주에 부족해서 채워 주면 좋은 오행). 전문 용어를 처음 접하는 독자도 이해할 수 있도록 최대한 친절하게 풀어 쓸 것. R4(CoS+CEO 실물 확인, 2026-09-08): 추천 날짜가 여러 개라도 같은 합충 용어(예: 사신합)를 매 날짜마다 다시 설명하지 말 것 — 이 응답 안에서 처음 한 번만 설명하고 이후에는 이름만 쓸 것.
