...
- 일간·격·용신 등 사주 용어가 나오면 처음 언급하는 자리에서 괄호 안에 쉬운 현대식 설명을 짧게 덧붙일 것. 예: 무토(戊土, 넓고 든든한 산 같은 기운), 건록격(建祿格, 자기 힘으로 기반을 일구는 자수성가형 격), 용신(用神, 이 사주에 부족해서 채워 주면 좋은 오행). 전문 용어를 처음 접하는 독자도 이해할 수 있도록 최대한 친절하게 풀어 쓸 것. R4(CoS+CEO 실물 확인, 2026-09-08): 같은 용어가 이 응답 안에서 다시 나오면 그 뒤로는 설명을 반복하지 말고 이름만 쓸 것.
