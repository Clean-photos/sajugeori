...
- 사주 용어(십성·신살·오행·일간·격 등 한자어)가 나오면 처음 언급하는 자리에서 괄호 안에 쉬운 현대식 설명을 짧게 덧붙일 것. 예: 괴강살(魁罡殺, 강한 카리스마와 승부 근성을 뜻하는 살), 무토(戊土, 넓고 든든한 산 같은 기운), 상관(傷官, 틀에 얽매이지 않는 자유로운 자기표현의 기운). 전문 용어를 처음 접하는 독자도 이해할 수 있도록 최대한 친절하게 풀어 쓸 것. R4(CoS+CEO 실물 확인, 2026-09-08): 같은 용어가 이 응답 안에서 다시 나오면 그 뒤로는 설명을 반복하지 말고 이름만 쓸 것.
- 나이 차이(연상·연하 등)는 이 분석의 근거가 아니므로 언급하지 말 것.
- §7-4(CoS+CEO 실물 확인, 2026-09-08): 종합 궁합 점수를 두고 "74점은 매우 잘 맞는다와 평범하다 사이 어딘가가 아니다"처럼 점수 해석 자체를 부정한 뒤, "100점짜리 궁합은 오히려 위험하다"는 식으로 낮은 점수가 더 낫다고 재반박하지 말 것 — 한 리포트 안에서 점수 체계를 스스로 무의미하게 만드는 자기모순이다. 점수는 있는 그대로(높을수록 좋음)로 일관되게 다루고, 점수가 낮은 영역은 "그래서 더 낫다"가 아니라 "이런 노력이 필요하다"는 방향으로 풀어 쓸 것.
...
${mutual.meToPartner.notes.map((n) => `- ${n}`).join("\n") || "- 특별한 상호작용 없음"}`.trim();
  ...
  // 2콜(각 3600토큰)로는 LLM 생성만 34초가 걸려(2026-08-25 실측) DB 왕복·콜드스타트를
  // 더하면 Vercel 60초 상한에 위험할 만큼 가깝다. 프리미엄 사주가 같은 구조에서 실제로
  // 타임아웃을 냈으므로 같은 방식으로 4콜로 쪼갠다 — 병렬이라 벽시계 시간은 가장 느린
  // 콜 하나에만 좌우되므로 쪼갤수록 총 소요가 줄어든다.
  const head = `당신은 명리학 궁합 대가입니다. 아래는 두 사람의 사주를 양방향으로 분석한 데이터입니다.
이 데이터로 유료 프리미엄 궁합 리포트의 일부를 작성하세요. 990원짜리 무료 버전과는 분량·깊이가 확연히
달라야 합니다.
