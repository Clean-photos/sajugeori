...
/**
 * 종별 행동 심리 — 동물행동학에서 일반적으로 알려진 경향을 담는다.
 ...
 * 다르게 지어내는 것보다 오히려 파급력이 크다. 구체적 비율·수치 단정은 넣지 않고
 * "알려져 있다"·"보고가 있다" 같은 헤지 표현만 쓴다.
...
> = {
  dog: {
    ...
    call: "강아지",
    traits: [
      "주인을 '안전기지(secure base)'로 삼는다고 알려져 있다. 주인이 곁에 있을 때 낯선 과제에 더 오래 도전하고, 낯선 사람이 있을 때는 같은 효과가 나타나지 않는다 — 주인에게만 해당하는 반응이라는 것이다.",
      "낯설거나 애매한 상황을 만나면 주인의 표정과 목소리를 살펴 자기 행동을 정하는 경향이 있다(사회적 참조). 낯선 물체를 마주치면 주인을 돌아보는 모습이 흔히 관찰된다고 한다.",
      "주인과 눈을 맞추는 동안 서로에게서 옥시토신이 오른다는 보고가 있다. 사람의 아기가 부모와 맺는 유대와 비슷한 회로로 설명되곤 한다.",
